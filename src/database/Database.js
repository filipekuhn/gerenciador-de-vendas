            ...
            .then(DB => {
              ...
              db.executeSql('SELECT 1 FROM city LIMIT 1').then(() => {
                  ...
                  db.transaction((tx) => {
                      ...
                      tx.executeSql('CREATE TABLE IF NOT EXISTS productsellingway (_id INTEGER PRIMARY KEY AUTOINCREMENT, idproduct INTEGER NOT NULL, idsellingway INTEGER NOT NULL, siteinclusiondate TEXT, saleprice DOUBLE, sitecommission DOUBLE, netprice DOUBLE, FOREIGN KEY (idproduct) REFERENCES product(_id), FOREIGN KEY(idsellingway) REFERENCES sellingway(_id));');
                      ...
                      tx.executeSql('CREATE TABLE IF NOT EXISTS sale (_id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, idcustomer INTEGER, idsellingway INTEGER, observations TEXT, saleprice DOUBLE, finalprice DOUBLE, amountpaid DOUBLE, pendingpayment BOOLEAN NOT NULL, FOREIGN KEY (idcustomer) REFERENCES customer (_id), FOREIGN KEY (idsellingway) REFERENCES sellingway (_id));');
                      tx.executeSql('CREATE TABLE IF NOT EXISTS productsale (_id INTEGER PRIMARY KEY AUTOINCREMENT, idsale INTEGER, idproductsellingway INTEGER, productprice DOUBLE, netprice DOUBLE, FOREIGN KEY (idsale) REFERENCES sale (_id), FOREIGN KEY (idproductsellingway) REFERENCES productsellingway (_id));');
                  }).then(() => {
